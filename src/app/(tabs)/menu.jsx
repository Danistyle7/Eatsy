...
const MenuScreen = () => {
  ...
  const filtrarPorCategoria = (categoria) =>
    sampleData.filter(item => item.categoria === categoria && item.nombre.toLowerCase().includes(busqueda.toLowerCase()));

  return (
    <ScrollView style={{ flex: 1, backgroundColor: 'white', paddingHorizontal: 16, paddingTop: 8 }}>
      {/* Header */}
      ...
      <View style={{ marginBottom: 10 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <Text style={{ color: '#EF6C00', fontSize: 24, fontWeight: 'bold' }}>Menu</Text>
          <BotonNaranja titulo="Agregar" onPress={() => console.log('Agregar presionado')} />
        </View>
